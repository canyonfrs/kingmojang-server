...
class CreatorInfoServiceTest : BehaviorSpec({
    ...
    Given("정상적인 크리에이터 정보를 생성하는 요청을 보내는 경우") {
        ...
        When("해당 요청으로 크리에이터 정보를 생성하면") {
            val actual = creatorInfoService.createCreatorInfo(MEMBER_ID, createCreatorInfoRequest())
    ...
    Given("정상적인 크리에이터 정보를 수정하는 요청을 보내는 경우") {
        ...
        When("해당 요청으로 크리에이터 정보를 수정하면") {
            val actual = creatorInfoService.updateCreatorInfo(
                MEMBER_ID, createCreatorInfoRequest(
    ...
    Given("로그인한 유저가 크리에이터 정보를 조회하는 요청을 보내는 경우") {
        val creatorInfo = createCreatorInfo()
        val creator = createMember(memberType = MemberType.CREATOR)
        creator.createCreatorInformation(creatorInfo)
        val followerId = 2L

        every { memberRepository.findByIdOrNull(MEMBER_ID) } returns creator
        every { followRepository.existsByCreatorIdAndFollowerId(MEMBER_ID, followerId) } returns true

        When("해당 요청으로 크리에이터 정보를 조회하면") {
            val actual = creatorInfoService.readCreatorInformation(followerId, MEMBER_ID)
